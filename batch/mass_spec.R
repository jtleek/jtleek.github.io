...
dat <- read.csv("./proteinMatrix.csv",row.names=1)
dat <- as.matrix(dat)
year <- read.csv("./batchVector.csv",row.names=1)
year <- year[,1]
grp <- read.csv("./outcomeVector.csv",row.names=1)
grp <- grp[,1]
...
# Assumes you start with a matrix of genes called "dat", an outcome variable "grp",
# and the surrogate, which I've called "year" (you could replace year with batch)
...
# RV coefficient between year model matrix and
# grp model matrix

ymat <- model.matrix(~ as.factor(year))
gmat <- model.matrix(~ as.factor(grp))
coeffRV(ymat,gmat)$rv
